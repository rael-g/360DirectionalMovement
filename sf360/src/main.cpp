...
struct Config
{
    ...
    bool  resetAoParar = true;      // esquece a direcao ao parar
    float passoMax = 0.38f;         // radianos por atualizacao; 0 = teleporte
    float velMin = 0.01f;           // Speed acima do qual conta como movimento
};
...
void CarregaConfig()
{
    ...
    while (std::fgets(linha, sizeof linha, f)) {
        ...
        else if (!std::strcmp(chave, "resetAoParar")) g_cfg.resetAoParar = lig;
...
public:
    void Run() override
    {
        ...
        if (!m_pulo) m_pulo = Interna("iSyncJumpState");
        ...
        if (vel <= g_cfg.velMin || pp >= 0.5f) {
            ...
            if (g_cfg.resetAoParar) m_ultimoDir = -1.0f;
            return;
        }
        ...
        if (m_pulo) {
            float pulo = 0.0f;
            // No pulo o rumo nao representa a intencao do jogador. Nao atualiza
            // o alvo, mas segue segurando o ultimo: soltar e que torcia o corpo
            // sempre para o mesmo lado.
            pulando = LeVariavel<float, kVtFloat>(holder, m_pulo, pulo)
                      && pulo != 0.0f;
        }
        ...
        if (dadoNovo && !pulando && temCam) {
            if (!m_emMovimento) {
                ...
                if (m_assentado && g_cfg.recapturaTroca) {
                    float dd = dir - m_dirRef;
                    if (dd < 0.0f) dd = -dd;
                    if (dd > 0.5f) dd = 1.0f - dd;
                    if (dd > kTrocouDirecao) {
                        m_assentado = false;
                        m_rumoAnt = rumoAgora;
                    }
                }
                if (!m_assentado && g_cfg.recapturaAssenta) {
                    if (fabs(Encurta(rumoAgora - m_rumoAnt)) < kRumoEstavel) {
...
private:
    ...
    struct Nota_t { int cod; float ang, erro, rumo, med, desl, dir, cam, desv, v0, vel; long gan; };
    ...
    void Despeja()
    {
        ...
        for (std::size_t i = 0; i < m_nNotas; ++i) {
            ...
            Loga("  %2zu cod=%d ang=%7.1f erro=%7.1f rumo=%7.1f "
                 "med=%7.1f d=%6.3f dir=%6.3f desv=%7.1f v0=%7.1f vel=%6.2f gan=%ld",
                 i, n.cod, n.ang, n.erro, n.rumo, n.med, n.desl, n.dir,
                 n.desv, n.v0, n.vel, n.gan);
        }
        m_nNotas = 0;
    }
    ...
    static constexpr float kTrocouDirecao = 0.06f;
...
__declspec(dllexport) bool SFSEPlugin_Load(const SFSEInterface* sfse)
{
    ...
    Loga("config: ganchoPost=%d recapturaAssenta=%d recapturaTroca=%d "
         "resetAoParar=%d passoMax=%.3f velMin=%.3f",
         g_cfg.ganchoPost, g_cfg.recapturaAssenta, g_cfg.recapturaTroca,
         g_cfg.resetAoParar, g_cfg.passoMax, g_cfg.velMin);
